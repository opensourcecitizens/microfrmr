...
const [marketPrices, setMarketPrices] = useState('');
  ...
  useEffect(()=>{
     const doSomething = async()=>{
        console.log("Dashboard doSomething filteredCards=",filteredCards);
        let filteredSummaries = await ApiCaller.getProductSummaryList()
           .then((summaries)=>{
        ...
        let filteredDetails = [];
        filteredSummaries?.flatMap(async(val)=>{
           let ret = await ApiCaller.getProductDetails(val.id);
           filteredDetails.push(ret[0]);
           //setFilteredCards([...filteredDetails,ret[0]]);
           setFilteredCards([...filteredDetails]);
           console.log("Dashboard filteredDetails=",filteredDetails);
        });
     }
     doSomething();
  ...
  useEffect(()=>{
    // load crops and animals
    ApiCaller.getProductSummaryList().then((prodList)=>{
